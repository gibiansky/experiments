...
class NeuralNetwork(object):
    ...
    def __init__(self, name = None, cache_dir = "weights"):
        ...
        self.cacher = DiskCacher(cache_dir + "/" + self.name)
        self.name_context = {}
        self.layers = []
    ...
    def create(self, **params):
        ...
        target = output_data.type("target")
        cost = self.objective(input_data, output_data, target)
        ...
        layer_outputs = [layer.layer_output() for layer in self.layers]

        # Compile all Theano functions
        logging.info("Compiling.")
        self.theano_get_output   = function(inputs=[input_data],          outputs=output_data)
        self.theano_get_layers   = function(inputs=[input_data],          outputs=layer_outputs)
        self.theano_train        = function(inputs=[input_data, target],  outputs=cost, updates=updates)
        self.theano_get_cost     = function(inputs=[input_data, target],  outputs=cost)
        logging.info("Done compiling.")

        # Ready to train
        self.initialized = True

    def learn(self, batch, target):
        """
        ...
        return self.theano_train(batch, target)

    def cost(self, batch, target):
        ...
        return self.theano_get_cost(batch, target)
...
class NeuralNetworkClassifier(NeuralNetwork):
    def objective(self, inp, out, target):
        return binary_crossentropy(out, target).mean()

    def test(self, test_set):
        test_size, test_data, test_labels, test_encoded = test_set
...
class NeuralNetworkAutoencoder(NeuralNetwork):
    def __init__(self, name = None, cache_dir = "weights", denoising = False, corruption = None):
        super(NeuralNetworkAutoencoder, self).__init__(name, cache_dir)
        self.denoising = denoising
        if self.denoising and corruption is None:
            self.corruption = 0.2

    def objective(self, inp, out, target):
        return tensor.mean((out - target) ** 2)

    def test(self, test_set):
        test_size, test_data, test_labels, test_encoded = test_set

        out = self.output(test_data)
        error = numpy.mean((out - test_data) ** 2)
        logging.info("Mean Error: %.2f" % error)
        logging.info("Input mean:  %.2f and standard deviation %.2f" % (numpy.mean(test_data), numpy.std(test_data)))
        logging.info("Output mean: %.2f and standard deviation %.2f" % (numpy.mean(out), numpy.std(out)))

    def learn(self, batch, labels=None):
        if not self.initialized:
            raise Exception("Neural network must be initialized prior to training.")

        return self.theano_train(self.corrupt(batch), batch)

    def cost(self, batch, labels=None):
        if not self.initialized:
            raise Exception("Neural network must be initialized prior to evaluation.")

        return self.theano_get_cost(self.corrupt(batch), batch)

    def corrupt(self, batch):
        if not self.denoising:
            return batch
        return batch * array(numpy.random.binomial(size=batch.shape, n=1, p=1 - self.corruption), dtype=float32)
...
class NeuralNetworkRegressor(NeuralNetwork):
    ...
    def test(test_set):
        test_size, test_data, test_outs, test_encoded = test_set

        out = self.output(test_data)
        error = numpy.mean((out - test_outs) ** 2)
        logging.info("Mean Error: %.2f" % error)
...
class NeuralNetworkLayer(object):
    ...
    def load_layer(self):
        ...
        for param in self.params:
            ...
            if self.cacher.cached(filename):
                ...
                logging.info("Randomly initialized parameter %s in layer '%s'" % (param.name, self.name))
    ...
    def copy_weights(self, source_layer):
        """ Initialize weights using weights from another layer object. """
        # Copy over parameters
        logging.info("Copied params from %s: %s" % (source_layer.network.name, source_layer.name))
        for (my_param, source_param) in zip(self.params, source_layer.params):
            my_param.set_value(source_param.get_value())
...
class InputLayer(NeuralNetworkLayer):
    ...
    def initialize(self, network, previous):
        ...
        if len(self.shape) == 3:
            ...
            self.output = tensor.tensor3("input")
...
class NormalizationLayer(NeuralNetworkLayer):
    ...
    def initialize(self, network, previous):
        ...
        self.scale = numpy.select([stdev != 0, stdev == 0], [1 / (stdev + (stdev == 0)), 0])

        # Take any input and shift and scale it.
        # The data shape remains the same.
        self.output = (previous.output - self.image_mean) * self.scale
...
class FullyConnectedLayer(NeuralNetworkLayer):
    ...
    """
    def __init__(self, neurons, name=None, activation=tensor.tanh, dropout=False, shape=None):
        self.name = name
        self.dropout = dropout

        # Allow strings to be used as activation functions for standard variations
        if hasattr(activation, '__call__'):
            ...
            if activation in ["sigmoid", "softmax", "logistic"]:
                self.activation = softmax

        # Allow specific shapes on the outputs
        if shape is None:
            self.shape = (neurons,)
        elif numpy.prod(shape) != neurons:
            raise Exception("Number of neurons (%d) does not match shape (%s)" % (neurons, str(shape)))
        else:
            self.shape = shape
    ...
    def layer_output(self):
        ...
        """
        # Data, arranged as a vector. We need two dimensions in the output because the first is the batch size.
        x = self.previous.layer_output().flatten(ndim=2)

        # Compute f(Wx+B)
        output = self.activation(dot(self.w, x.transpose()).transpose() + self.b)

        # If dropout is enabled, randomly set half to zero
        if self.dropout:
        ...
        # Reshape output to be correct dimensions. Remember that first dimension is batch size.
        datashape = (self.network.batch_size,) + self.shape
